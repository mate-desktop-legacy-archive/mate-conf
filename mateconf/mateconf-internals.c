...
mateconf_value_from_corba_value(const ConfigValue* value)
{
  ...
  switch (value->_d)
    ...
    case PairVal:
      ...
      break;
    default:
      mateconf_log(GCL_DEBUG, "Invalid type in %s", G_STRFUNC);
      return NULL;
    }
                                        ...
                                        ConfigValue      *cv)
{
  ...
  switch (value->type)
    ...
    default:
      cv->_d = InvalidVal;
      mateconf_log(GCL_DEBUG, "Unknown type in %s", G_STRFUNC);
      break;
    }
}
                                          ...
                                          ConfigSchema      *cs)
{
  ...
  cs->gettext_domain = CORBA_string_dup (mateconf_schema_get_gettext_domain (sc) ? mateconf_schema_get_gettext_domain (sc) : "");
  ...
  if (*cs->gettext_domain != '\0')
    {
      if (!g_utf8_validate (cs->gettext_domain, -1, NULL))
        mateconf_log (GCL_ERR, _("Invalid UTF-8 in gettext domain for schema"));
      else
...
/*
 * Configuration files (yikes! we can't store our configuration in MateConf!)
 */
         ...
         GString  *failure_log)
{
        ...
        /* if the bus isn't running and we don't want to start mateconfd then
...
static ConfigServer
mateconf_get_server (gboolean  start_if_not_found,
                  GString  *failure_log)
{
  ConfigServer server;
  char *ior;
                       ...
                       GError  **error)
{
  ...
  if (server == CORBA_OBJECT_NIL &&
    ...
    g_set_error (error,
                 ...
                 _("Failed to contact configuration server; some possible causes are that you need to enable TCP/IP networking for MateCORBA, or you have stale NFS locks due to a system crash. See http://wiki.mate-desktop.org/docs:mate-conf for information. (Details - %s)"),
